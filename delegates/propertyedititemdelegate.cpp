...
void PropertyEditItemDelegate::destroyEditor(QWidget *editor,
...
QWidget *PropertyEditItemDelegate::createEditor(
  ...
  if (UtilJsonStringProperty::isTypeBufferContainImage1FieldFromProperty(
          index.data().toByteArray())) {
    ImageFileSetPropertyEditor *e = new ImageFileSetPropertyEditor(
        const_cast<PropertyEditItemDelegate *>(this), parent);
...
void PropertyEditItemDelegate::paint(QPainter *painter,
  ...
  if (UtilJsonStringProperty::isTypeBufferContainImage1FieldFromProperty(
    ...
    if (UtilJsonStringProperty::isExistImage1FieldFromProperty(
      ...
      QRect r(marginleft, marginup, option.rect.width() / 6,
      ...
      QFontMetrics f(font);
      auto elided_text = f.elidedText(
          UtilJsonStringProperty::extractFileNameFromPathImage1Field(
              index.data().toByteArray()),
          Qt::ElideRight, option.rect.width() - r.width() - 4);
      QRectF trs(r.right() + 4, r.y(), option.rect.width() - r.width() - 4,
                 r.height());
